...
namespace taskt.Core.Server
{
    ...
    public static class LocalTCPClient
    {
        ...
        public static AutomationEngineInstance AutomationInstance;

        private static AutomationEngineInstance _executeCommandEngine;
        private static Logger _automationLogger;
        private static TcpListener _tcpListener;
        private static LocalListenerSettings _listenerSettings;
        private static frmScriptBuilder _associatedBuilder;
        private static int _port;

        static LocalTCPClient()
        {
            _automationLogger = new Logging().CreateLogger("Automation Client", Serilog.RollingInterval.Day);
            _executeCommandEngine = new AutomationEngineInstance();
        }

        public static void Initialize(frmScriptBuilder builder)
        {
            _associatedBuilder = builder;
            _automationLogger.Information("Automation Listener Initializing");

            AutomationInstance = new AutomationEngineInstance();
        ...
        private static void ProcessRequest(string data, string[] messageContent, NetworkStream stream)
        {
            ...
            if (data.StartsWith("POST /ExecuteScript") || data.StartsWith("POST /AwaitScript"))
            {
                ...
                if (isFileLocation)
                {
                    var fallbackPath = Path.Combine(Folders.GetFolder(Folders.FolderType.ScriptsFolder), dataParameter);
            ...
            else if (data.StartsWith("POST /ExecuteCommand"))
            {
                ...
                try
                {
                    ...
                    var scriptAction = new Script.ScriptAction() { ScriptCommand = (ScriptCommand)command };
                                                ...
                                                string scriptData = "", string awaitPreference = "")
        {
            ...
            switch (parameterType)
            {
                case "Run Raw Script Data":
                    ...
                    break;
                case "Run Local File":
                    string fallbackPath = Path.Combine(Folders.GetFolder(Folders.FolderType.ScriptsFolder), scriptData);
